...
//日期+时间选择（选择器显示，选择器赋值，标题，从哪年开始，到哪年结束）
function kc_datetime(select,selectval, title, startY, endY) {
    var date = new Date;
    ...
    var mobileSelect4 = new MobileSelect({
        ...
        transitionEnd: function (indexArr, data) {
            ...
            if (nowyPosition != oldyPosition) {	//判断是否是年份滑动，如果是，则月份和天都归为01
                mobileSelect4.locatePosition(1, 0);
                mobileSelect4.locatePosition(2, 0);
            }
        ...
        callback: function (indexArr, data) {
            //点击确认选择后的回调
            $(select).html(data[0].value + data[1].value + data[2].value + " " + data[3].value + data[4].value);
            $(selectval).val(data[0].value + data[1].value + data[2].value + " " + data[3].value + data[4].value);
        }
    });
}
//日期选择（选择器显示，选择器赋值，标题，从哪年开始，到哪年结束）
function kc_date(select, selectval, title, startY, endY) {
    var date = new Date;
    var year = date.getFullYear();		//初始年份
    var month = 01;		//初始月份
    var days = new Date(year, month, 0).getDate();  //1970年01月有多少天
    ...
    var mobileSelect4 = new MobileSelect({
        ...
        callback: function (indexArr, data) {
            //点击确认选择后的回调
            $(select).html(data[0].value + data[1].value + data[2].value);
            $(selectval).val(data[0].value + data[1].value + data[2].value);
        }
    });
}
//性别选择（选择器显示，选择器赋值)
function kc_sex(select, selectval) {
    var mobileSelect1 = new MobileSelect({
        ...
        callback: function (indexArr, data) {
            $(selectval).val(data[0].id);
        }
    });
}
function getObject(star, end, title) {
    var json = [];
